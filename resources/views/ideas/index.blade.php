<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if (session('message'))
                <div class="bg-{{ session('color') }} text-center rounded-md p-2 mb-4">
                    <span class="text-white text-xl">{{ session('message') }}</span>
                </div>
            @endif
            <div class="overflow-hidden shadow-sm sm:rounded-lg mb-4">
                <div class="p-6 text-gray-900 dark:text-gray-100s space-x-8">
                    <a href="{{ route('idea.create') }}"
                        class="px-4 py-4 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-sm text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700">{{ __('Agregar') }}</a>
                    <a href="{{ route('idea.index', ['filtro' => 'mis-ideas']) }}"
                        class="px-4 py-4 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-sm text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700">{{ __('Mis ideas') }}</a>
                    <a href="{{ route('idea.index', ['filtro' => 'las-mejores']) }}"
                        class="px-4 py-4 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-sm text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700">{{ __('Las Mejores') }}</a>
                </div>
            </div>
                    ...
                    <div class="p-6 flex space-x-2">
                        ...
                        <div class="flex-1 pl-3">
                            <div class="flex justify-between items-center">
                                <div>
                                    <span class="text-gray-800 dark:text-gray-100 ms-3">{{ $idea->user->name }}</span>
                                    <small
                                    ...
                                    @unless ($idea->created_at->eq($idea->updated_at))
                                        <small class="text-sm dark:text-green-400"> &middot; {{ __('Editado') }}</small>
                                    @endunless
                                </div>
                                @auth
                                    <x-dropdown>
                            ...
                            <p class="mt-3 text-lg text-gray-900 dark:text-gray-100 ms-3">{{ $idea->title }}</p>
                            <small class="text-sm text-gray-400 flex mt-3 ms-3">
                                ...
                                <span class="ml-2 ms-3">{{ $idea->likes }}</span>
                            </small>
                        </div>
                    </div>
                @empty
                    <h2 class="text-white p-4">No existen ideas almacenadas</h2>
                @endforelse
            </div>
        </div>
    </div>
